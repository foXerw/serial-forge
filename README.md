...
Top-right buttons: 「📖 使用说明」 (help) and 「✎ 编辑协议」 (edit protocol).
...
```bash
dotnet test      # 67 tests — Core engine, framer, transport, editor VMs
```
...
| `SerialForge.Transport` | Serial I/O (`SerialTransport`), `Framer`, `FrameDispatcher` (+ Phase-2 `Await` seam) |
| `SerialForge.App` | WPF / MVVM (CommunityToolkit.Mvvm): connection bar, command panel, hex log, **protocol editor**, help |
...
- **Phase 2** — firmware-upgrade Flow engine, plugging into the existing
  `FrameDispatcher.Await` seam (start → chunked transfer → end + verify).
...
```bash
dotnet test      # 67 个测试 —— 引擎、成帧、传输、编辑器 VM
```
...
| `SerialForge.Transport` | 串口 I/O（`SerialTransport`）、`Framer`、`FrameDispatcher`（含 Phase-2 `Await` 接缝） |
| `SerialForge.App` | WPF / MVVM（CommunityToolkit.Mvvm）：连接栏、命令面板、收发日志、**协议编辑器**、使用说明 |

Core 层零 WPF、零串口依赖，引擎与成帧层可脱离硬件与 UI 完整单测。

### 路线图

- **Phase 2** —— 固件升级 Flow 引擎，接入现有的 `FrameDispatcher.Await` 接缝
  （start 包 → 分块 transfer → end 包 + 整包校验）。
